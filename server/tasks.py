...
class SubjectResearchTasks():
    ...
    def manage_research(self, agent: Agent, subject: list[str], topics: list[str], tasks: list[Task]):
        return Task(
            description=dedent(f"""Based on the list of companies {subject} and the positions {topics},
    ...
    def subject_research(self, agent: Agent, subject: str, topics: list[str]):
        return Task(
            description=dedent(f"""Research the position {subject} for the {topics} company. 
                For each position, 
